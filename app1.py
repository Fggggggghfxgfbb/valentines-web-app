...
followers = dbs.Table("followers",
                      dbs.Column('followerid', dbs.Integer, dbs.ForeignKey('user.id')),
                      dbs.Column('followedid', dbs.Integer, dbs.ForeignKey('user.id'))
                      )
...
class User(dbs.Model, UserMixin):
    ...
    followed = dbs.relationship(
        'User', secondary=followers,
        primaryjoin=(followers.c.followerid == id),
        secondaryjoin=(followers.c.followedid == id),
        backref=dbs.backref('followers', lazy='dynamic'), lazy='dynamic')
    ...
    def followedposts(self):
        followed = Post.query.join(followers, (followers.c.followedid == Post.userid)).filter(
            followers.c.followerid == self.id).order_by(Post.postdate.desc())
        own = Post.query.filter_by(userid=self.id)
        return followed.union(own).order_by(Post.postdate.desc())
...
class Post(dbs.Model):
    ...
    pname = dbs.Column(dbs.String(20), nullable=False)
    postdate = dbs.Column(dbs.DateTime, nullable=False)
    comments = dbs.relationship('Comment', lazy='dynamic', cascade="all, delete")
...
def index1(username):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            dbs.session.commit()
            return redirect(url_for('index1', username=username))

    comments = Comment.query.order_by(Comment.postdate.desc())

    return render_template('acc.html', posts=posts, form=form, comments=comments, username=username, user=user)
...
@app.route("/index21", methods=['GET', 'POST'])
@login_required
def index2():
    posts = current_user.followedposts().all
    form = commentform()
    idiij = 0
    if request.method == 'POST':
        if form.validate_on_submit():
...
@app.route("/index/post/<post_id>", methods=['GET', 'POST'])
@login_required
def delete(post_id):
...
@app.route("/index/comment/<commentid>", methods=['GET', 'POST'])
@login_required
def deletecomment(commentid):
    comment = Comment.query.filter_by(id=commentid).first()

    dbs.session.delete(comment)
    dbs.session.commit()
    return redirect(url_for('index'))
...
@app.route("/index/unfollow/<username>", methods=['GET', 'POST'])
@login_required
...
@app.route("/index/follow/<username>", methods=['GET', 'POST'])
@login_required
def follow(username):
    if request.method == 'POST':
        user = User.query.filter_by(username=username)
